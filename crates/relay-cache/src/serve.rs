...
//!
//! `/cache-health` is the public readiness shape (`{ready, regions[]}`) —
//! nginx proxies it. The operational internals (memory pressure, stream
//! counters, per-table row counts) live on `/internal/stats`, which
//! nginx intentionally does NOT proxy; reachable only on the loopback
//! bind (`127.0.0.1:8089`).
//!
//! One WebSocket is served alongside the HTTP API:
//! `/internal/dim-buildings/ws` pushes the building-entity-id set of
//! housing interior dimensions. nginx proxies it publicly (no PII, no
//! admin surface — just u64 ids per `(region, dimension)`); the primary
//! consumer is `bitcraft-streamd` on a separate host, but any client may
//! connect. The rest of `/internal/*` (notably `/internal/stats`) stays
//! loopback-only — the nginx config enforces that with an explicit
//! `location /internal/ { return 404; }` catch behind the dim-buildings
//! route. See `DIM-BUILDINGS-WS.md` for the wire protocol.
...
pub async fn serve(
    ...
    let app = Router::new()
        ...
        // Loopback-only operational diagnostics — nginx does NOT proxy
        // this path, so it is unreachable from the public Internet.
        .route("/internal/stats", get(internal_stats))
        // Housing-interior building push WS. nginx proxies this publicly
        // (no PII / no admin surface — just u64 ids per dimension); the
        // rest of /internal/* stays loopback-only via an nginx 404 catch.
        .route("/internal/dim-buildings/ws", get(stream::dim_buildings_ws))
...
/// Public readiness shape — `{ready, regions:[{region, ready}]}`. The
/// operational internals (memory pressure, stream counters, per-table
/// row counts) are deliberately not exposed here; see `/internal/stats`.
...
/// Loopback-only diagnostics. Same `Fleet` state as `/cache-health` plus
/// the per-table row counts and the interest-hub stream counters. Not
/// proxied by nginx — reachable only on the loopback bind.
async fn internal_stats(State(fleet): State<Fleet>) -> impl IntoResponse {
    let pressure = fleet.memory_pressure.load(Ordering::Relaxed);
    let mut regions = Vec::with_capacity(fleet.shards.len());
    for shard in &fleet.shards {
        let s = shard.store.read();
    ...
    no_store_json(json!({
        "ready": !pressure && regions.iter().all(|r| r["ready"].as_bool() == Some(true)),
        "memory_pressure": pressure,
        "streams": {
            "active_connections": fleet.interest.active_connections(),
            "active_leases": fleet.interest.active_leases(),
            // Legacy alias (= active_leases) for older dashboards.
            "active": fleet.interest.active_leases(),
...
/// Per-table row counts for one region store. Shared by `/internal/stats`.
fn region_rows(s: &RegionStore) -> Value {
    json!({
        "claim": s.claim.len(),
...
fn respond_claim_members(headers: &HeaderMap, body: pb::ClaimMemberList) -> Response {
    if wants_protobuf(headers) {
        ...
        no_store_json(claim_members_to_json(&body)).into_response()
    }
}

pub(crate) fn claim_members_to_json(body: &pb::ClaimMemberList) -> Value {
...
fn deposit_from_claim_slot(s: &RegionStore, slot: u32) -> Option<pb::HexiteDeposit> {
    ...
    let (respawn_at, status) = match resource_slot {
        Some(rs) => {
            ...
            // Authoritative respawn clock: `resource_growth_timer.scheduled_at`
            // (the scheduled reducer row). Fall back to legacy
            // `growth_state.end_timestamp` if the timer row is absent.
            let respawn_micros = s
                .growth_timer
                .scheduled_at_micros(resource_entity_id)
                .or_else(|| s.growth.end_timestamp_micros(resource_entity_id));
            if let Some(end_micros) = respawn_micros {
                (
                    Some(format_rfc3339_millis(end_micros)),
                    None, // timer present ⇒ respawning (client contract)
                )
            } else if depleted {
                // Depleted but no growth row visible yet (rare race, or a
                // region still mid self-heal after reconnect).
...
/// Unified claim roster: roles + skills + hexcoins (HTTP + protobuf).
pub(crate) fn build_claim_members(fleet: &Fleet, pk: u64) -> Option<pb::ClaimMemberList> {
    // Claim membership lives on the claim's region shard, but
    // `experience_state` / personal bags / login timestamps often live on
    // each player's home region.
    ...
    let mut members: Vec<pb::ClaimMember> = Vec::new();
    ...
    for shard in &fleet.shards {
        ...
        for &slot in s.claim_member.by_claim(pk) {
            ...
            members.push(pb::ClaimMember {
                ...
                skills,
                hexcoins: sum_player_hexcoins(&s, player_id),
            });
        }
        break;
    }

    let claim = claim_summary?;

    for m in &mut members {
        let player_id = m.player_entity_id;
        for shard in &fleet.shards {
            let s = shard.store.read();
            ...
            if m.last_login_timestamp.is_none() {
                m.last_login_timestamp = s.player_state.last_login_timestamp(player_id);
            }
            if m.last_active_timestamp.is_none() {
                m.last_active_timestamp = s.mobile_entity.last_active_timestamp(player_id);
            }
            let home_hex = sum_player_hexcoins(&s, player_id);
            if home_hex > m.hexcoins {
                m.hexcoins = home_hex;
            }
        }
    }
...
async fn claim_citizens(
    ...
    let citizens = full
        .members
        .into_iter()
        .map(|m| pb::ClaimCitizen {
            entity_id: m.player_entity_id,
            user_name: m.user_name,
            skills: m.skills,
            last_login_timestamp: m.last_login_timestamp,
            last_active_timestamp: m.last_active_timestamp,
        })
        .collect();
...
async fn claim_hexcoins(
    ...
    let mut members: Vec<pb::MemberHexcoins> = full
        .members
        .into_iter()
        .map(|m| pb::MemberHexcoins {
            player_entity_id: m.player_entity_id,
            user_name: m.user_name,
            hexcoins: m.hexcoins,
            has_storage_access: m.inventory_permission,
        })
        .collect();
    members.sort_by(|a, b| b.hexcoins.cmp(&a.hexcoins));
    respond_claim_hexcoins(
        &headers,
        pb::ClaimHexcoins {
            claim: full.claim,
            members,
...
/// Lightweight sibling of [`collect_housing_buildings`]: returns just the
/// filtered, ascending-sorted building entity ids in a housing interior
/// dimension (storage buildings only — same `include_in_claim_inventory`
/// filter as the HTTP `/player/:id/housing` join). Used by the loopback
/// `/internal/dim-buildings/ws` handler on every watch bump, so it must be
/// cheap: no protobuf, no nickname/item aggregation, just the id set.
pub(crate) fn housing_building_ids(s: &RegionStore, entrance_dimension_id: u32) -> Vec<u64> {
    let mut ids: Vec<u64> = s
        .location_dim
        .entities_in(entrance_dimension_id)
        .iter()
        .copied()
        .filter(|&eid| {
            s.building.find(eid).is_some_and(|slot| {
                s.building_desc
                    .include_in_claim_inventory(s.building.building_description_id[slot as usize])
            })
        })
        .collect();
    ids.sort_unstable();
...
mod tests {
    ...
    /// Build a minimal Fleet with one shard whose `ready` flag we control.
    fn fleet_with(ready: bool) -> Fleet {
        use crate::shard::ShardHandle;
        use std::sync::atomic::AtomicBool;
        let mut store = RegionStore::empty(14);
        store.ready = ready;
        let shard = Arc::new(ShardHandle {
            region: 14,
    ...
    async fn cache_health_exposes_only_ready_shape() {
        ...
        assert_eq!(v["regions"][0]["region"], json!(14));
        assert_eq!(v["regions"][0]["ready"], json!(true));
        for key in ["memory_pressure", "streams", "rows"] {
            assert!(v.get(key).is_none(), "public /cache-health leaked `{key}`");
        }
        assert!(v["regions"][0].get("rows").is_none());
    }
    ...
    async fn internal_stats_exposes_internals() {
        ...
        assert_eq!(v["streams"]["active"], json!(0)); // legacy alias
        assert_eq!(v["regions"][0]["region"], json!(14));
        // Every per-table row count key is present.
        assert!(v["regions"][0]["rows"].get("claim").is_some());
        assert!(v["regions"][0]["rows"].get("storage_log_state").is_some());
    }
    ...
    /// Regression: a depleted deposit with no `growth_state` row but a live
    /// `resource_growth_timer` row must surface a `respawn_at`. Before the
    /// fix, these deposits fell through to `status:"respawning"` (or, when
    /// resource_id didn't flip, `"harvestable"`) because the timer table
    /// wasn't subscribed.
    #[test]
    fn deposit_from_claim_slot_uses_growth_timer_when_no_growth_state() {
        ...
        s.growth_timer.upsert(ResourceGrowthTimerRow {
            entity_id: 999,
            scheduled_at_micros: Some(1_784_475_313_483_000),
            growth_recipe_id: 801_862_900,
        });
        let slot = s.claim.find(100).unwrap();
        let dep = deposit_from_claim_slot(&s, slot).unwrap();
        assert_eq!(
            dep.respawn_at.as_deref(),
            Some("2026-07-19T15:35:13.483Z"),
            "respawn_at must come from resource_growth_timer.scheduled_at"
        );
        assert!(dep.status.is_none());
    }

    /// The scheduled timer is authoritative; if a stale `growth_state` row
    /// also exists, the timer's `scheduled_at` wins.
    #[test]
    fn deposit_from_claim_slot_prefers_growth_timer_over_growth_state() {
        use crate::decode::{
            ClaimLocalRow, ClaimRow, GrowthRow, ResourceGrowthTimerRow, ResourceRow,
            DEPLETED_HEXITE_DEPOSIT_RESOURCE_ID,
        };
        ...
        s.growth.upsert(GrowthRow {
            entity_id: 999,
            end_timestamp_micros: 1_000_000_000_000_000,
            growth_recipe_id: 1,
        });
        // Authoritative timer (newer timestamp).
        s.growth_timer.upsert(ResourceGrowthTimerRow {
            entity_id: 999,
            scheduled_at_micros: Some(1_784_475_313_483_000),
            growth_recipe_id: 801_862_900,
        ...
        assert_eq!(
            dep.respawn_at.as_deref(),
            Some("2026-07-19T15:35:13.483Z"),
            "timer.scheduled_at must win over growth_state.end_timestamp"
        );
        assert!(dep.status.is_none());
    }
