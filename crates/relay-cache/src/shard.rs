...
async fn session(
    ...
    let n_growth = building.growth.len();
    {
        ...
        *guard = building;
        ...
        interest.replace_region_members(&old_pairs, &new_pairs);
    }
    tracing::info!(
        target: "relay_cache::shard",
        region,
                ...
                match frame {
                    Ok(frame) => match frame.message {
                        ServerMessage::TransactionUpdate(tu) => {
                            if let Err(e) =
                                apply_transaction(store, schema, meta, &tu, interest)
                            {
                                tracing::warn!(
                                    target: "relay_cache::shard",
                                    region,
...
fn base_subscribe_queries() -> Vec<String> {
    vec![
        ...
        // (subscribed next). We keep both for overlap during any transition.
        format!("SELECT * FROM {GROWTH_TABLE}"),
        // Scheduled growth timers — one row per growing resource entity
        // (~28k/region, all resource types). `scheduled_at` (Time arm of
        // ScheduleAt) is the absolute completion timestamp and the source of
        // `respawn_at` for depleted Hexite Deposits.
        format!("SELECT * FROM {RESOURCE_GROWTH_TIMER_TABLE}"),
...
fn merge_subscribe_applied(
    ...
    for table in sa.rows.tables.iter() {
        ...
        apply_rows(store, schema, meta, name, &[], &rows, None, None)?;
    }
    for table in sa.rows.tables.iter() {
        let name: &str = table.table.as_ref();
        if name != LOCATION_TABLE {
...
fn apply_rows(
    ...
    match table {
        ...
        CLAIM_MEMBER_TABLE => {
            for row in deletes {
                ...
                if let Some(hub) = interest {
                    hub.forget_member(decoded.player_entity_id, decoded.claim_entity_id);
                }
                if let Some(t) = touches.as_deref_mut() {
                    t.claim_members(decoded.claim_entity_id);
                }
            ...
            for row in inserts {
                ...
                if let Some((old_player, old_claim)) = store.claim_member.player_claim(decoded.entity_id)
                {
                    ...
                    if let Some(t) = touches.as_deref_mut() {
                        if old_claim != decoded.claim_entity_id {
                            t.claim_members(old_claim);
                        }
                    }
                }
                let player = decoded.player_entity_id;
                let claim = decoded.claim_entity_id;
                store.claim_member.upsert(decoded);
                if let Some(hub) = interest {
                    hub.note_member(player, claim);
                }
                if let Some(t) = touches.as_deref_mut() {
                    t.claim_members(claim);
                }
            }
        }
        ...
        RESOURCE_GROWTH_TIMER_TABLE => {
            for row in deletes {
                let decoded = decode::decode_resource_growth_timer_with_fields(
                    row,
                    &meta.growth_timer_fields,
                    meta.cols.growth_timer,
                    schema,
                )?;
                store.growth_timer.delete(decoded.entity_id);
            }
            for row in inserts {
                let decoded = decode::decode_resource_growth_timer_with_fields(
                    row,
                ...
                store.growth_timer.upsert(decoded);
            }
        }
        STORAGE_LOG_TABLE => {
            for row in deletes {
...
fn touch_inventory(
    ...
    if owner != 0 {
        if let Some(d_slot) = store.deployable.find(owner) {
            let dep_owner = store.deployable.owner_id[d_slot as usize];
...
fn touch_building(
    ...
    // The building itself changed (e.g. its building_description_id flipped
    // to a storage type, entering/leaving the `include_in_claim_inventory`
    // filter). Wake the dim-buildings WS subscribers for its current dim
    // so they re-scan and emit an added/removed delta. If the location row
    // for this entity hasn't landed yet this transaction, the next one will
    // catch it via `touch_location_entity`.
    let dim = store.location_dim.get_or_overworld(building_entity_id);
    if dim != OVERWORLD_DIMENSION {
        touches.dimension_buildings(store.region, dim);
    }
...
fn touch_location_entity(
    ...
    // A building (or any entity) moved into/out of this housing-interior
    // dimension — wake the dim-buildings WS subscribers for this region+dim.
    touches.dimension_buildings(store.region, dimension);
