...
impl ClaimMemberSoA {
    ...
    /// `(player_entity_id, claim_entity_id)` for every live membership row.
    pub fn player_claim_pairs(&self) -> Vec<(u64, u64)> {
        self.pk
            .values()
            .map(|&slot| {
                let i = slot as usize;
                (self.player_entity_id[i], self.claim_entity_id[i])
            })
            .filter(|&(p, c)| p != 0 && c != 0)
            .collect()
    }

    /// `(player_entity_id, claim_entity_id)` for a membership PK, if present.
    pub fn player_claim(&self, entity_id: u64) -> Option<(u64, u64)> {
        let &slot = self.pk.get(&entity_id)?;
        let i = slot as usize;
        Some((self.player_entity_id[i], self.claim_entity_id[i]))
    }
