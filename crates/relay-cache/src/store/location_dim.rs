...
//! Interior entity → dimension map from `location_state`.
//!
//! The base subscribe uses `WHERE dimension != 1`, so overworld rows are
//! not bulk-loaded. Hexite PK attaches may still upsert overworld
//! `location_state` rows here (deposit entity x/z). Buildings absent from
//! this map are treated as overworld (1).
//!
