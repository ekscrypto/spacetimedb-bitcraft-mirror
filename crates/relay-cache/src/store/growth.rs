...
//! Lookup store for `growth_state` — the **legacy** respawn snapshot.
//!
//! On current BitCraft builds this table is near-empty; the authoritative
//! respawn clock is `resource_growth_timer` (see `resource_growth_timer.rs`),
//! which `/deposits` prefers. This store remains as a fallback so any region
//! still writing `growth_state.end_timestamp` continues to surface a
//! `respawn_at`.
